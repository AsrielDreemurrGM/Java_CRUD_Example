 ...
 * 
 * <p>This class provides basic CRUD operations for entities of type {@code T}
 * that implement {@link Persistable}. It manages a two-level Map:
 * an outer Map from Class<T> to inner Maps, and inner Maps from unique ID strings to entities.</p>
 * 
 * <p>Subclasses must specify the entity Class type and
 * implement how to update an existing registered entity with new data.</p>
 *
...
public abstract class GenericMapDAO<T extends Persistable> implements IGenericDAO<T> {
	...
	/**
	 * Constructor initializes the storage map and ensures inner map for the entity class exists.
	 */
